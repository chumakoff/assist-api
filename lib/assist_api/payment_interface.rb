require 'uri'
require 'assist_api/checkvalue_generator'
...
module AssistApi
  class PaymentInterface
    ...
    def initialize(order_number, order_amount, extra_params = {})
      ...
      return unless AssistApi.config.checkvalue?
      params[:checkvalue] = generate_checkvalue(params)
    end

    def url
      uri = URI(AssistApi.config.endpoint + SERVICE_PATH)
    ...
    def default_params
      attrs = {merchant_id: AssistApi.config.merchant_id}
      ...
      .each do |key, value|
        attrs[value] = AssistApi.config[key] if AssistApi.config[key]
      end

      attrs.merge!(payment_methods_params) if AssistApi.config.payment_methods
      attrs
    end
    ...
    def payment_methods_params
      ...
      PAYMENT_METHODS_MAPPING.each do |key, value|
        attrs[value] = 0 unless AssistApi.config.payment_methods[key]
      end

      if AssistApi.config.payment_methods[:qiwi].is_a?(Hash)
        QIWI_METHODS_MAPPING.each do |key, value|
          attrs[value] = 0 unless AssistApi.config.payment_methods[:qiwi][key]
        end
      end
