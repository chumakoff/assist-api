  ...
  context "when successful response with error" do
    ...
    it "should raise an error with response error codes" do
      expect{ result.result }.to raise_error do |error|
        expect(error).to be_a AssistApi::Exception::APIError
...
shared_examples_for "response with checkvalue" do
  context "when secret word is set in the config" do
    before(:each) do
      ...
      expect(AssistApi.config).to be_checkvalue
    end

    context "when correct checkvalue" do
      subject { AssistApi::WebServices::OrderStatus.new("ok", extra_params) }
    ...
    context "when wrong checkvalue" do
      subject { AssistApi::WebServices::OrderStatus.new("wrong_checkvalue", extra_params) }

      it "should raise an error" do
        expect{ subject.result }.to raise_error(AssistApi::Exception::APIError,
                                                "Wrong checkvalue")
      end
    end
  end
...
shared_examples_for "correct params set" do
  ...
  it "should contain default params" do
    expect(subject).to include({merchant_id: AssistApi.config.merchant_id,
