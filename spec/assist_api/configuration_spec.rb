...
describe AssistApi::Configuration do
  let(:config) { AssistApi::Configuration.new }
  ...
  describe "#validate!" do
    ...
    context "when there are missing required options" do
      it "should raise an error" do
        AssistApi::Configuration::REQUIRED_OPTIONS.each do |option|
          ...
          expect{ config.validate! }.to raise_error do |error|
            expect(error).to be_a AssistApi::Exception::ConfigurationError
  ...
  describe "#endpoint" do
    context "when mode is production" do
      it "should return production endpoint" do
        ...
        expect(config.endpoint).to eq AssistApi::Configuration::ENDPOINTS[:production]
      end
    end
    ...
    context "when mode is test" do
      it "should return test endpoint" do
        config.mode = :test
        expect(config.endpoint).to eq AssistApi::Configuration::ENDPOINTS[:test]
      end
    end
